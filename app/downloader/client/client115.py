...
class Client115(_IDownloadClient):
    ...
    @classmethod
    def match(cls, ctype):
        return True if ctype in [cls.client_id, cls.client_name] else False
    ...
    def get_torrents(self, ids=None, status=None, **kwargs):
        ...
        if not ret:
            log.info(f"【{self.client_name}】获取任务列表错误：{self.downclient.err}")
            return tlist
        if tasks:
            for task in tasks:
                if ids:
    ...
    def add_torrent(self, content, download_dir=None, **kwargs):
        ...
        if isinstance(content, str):
            ...
            if not ret:
                log.error(f"【{self.client_name}】添加下载任务失败：{self.downclient.err}")
                return None
